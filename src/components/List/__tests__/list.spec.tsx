...
describe('List', () => {
  ...
  it('Order', () => {
    ...
    const { container, getByTestId, getByText } = render(<List />);

    fireEvent.change(getByTestId('select-order'), { target: { value: 'old' } });

    const firstItem = container.querySelector('li');

    expect(firstItem).toContainElement(getByText('Matleh'));

    fireEvent.change(getByTestId('select-order'), {
      target: { value: 'recent' },
    });

    const secondItem = container.querySelector('li');

    expect(secondItem).toContainElement(
      getByText('AssCo has revamped the theory of versioning'),
    );
  });
});
